...
namespace lpzrobots {
    ...
    void Sphererobot3MassesModConf::destroy() {
    ...
    const int Sphererobot3MassesMod::servono;

    /**
     *constructor
     **/
    Sphererobot3MassesMod::Sphererobot3MassesMod(const OdeHandle &odeHandle, const OsgHandle &osgHandle,
                                                 const Sphererobot3MassesModConf &conf,
     ...
     *constructor
     **/
    Sphererobot3MassesMod::Sphererobot3MassesMod(const OdeHandle &odeHandle,
    ...
    void Sphererobot3MassesMod::init() {
        created = false;
        objects.resize(Last);
    ...
    Sphererobot3MassesMod::~Sphererobot3MassesMod() {
        destroy();
    ...
    void Sphererobot3MassesMod::update() {
     ...
     **/
    int Sphererobot3MassesMod::getSensorsIntern(sensor *sensors, int sensornumber) {
        int len = 0;
        assert(created);
        ...
        if (conf.irInverseValue) {
            for (int i = len0; i < len; i++) {
                if (sensors[i] >= -1e6) sensors[i] = (conf.irsensorscale - sensors[i]) / conf.irsensorscale;
                else sensors[i] = -sensors[i] - conf.irsensorscale; // happen? may assert it
            }
        }
    ...
    void Sphererobot3MassesMod::setMotorsIntern(const double *motors, int motornumber) {
    ...
    void Sphererobot3MassesMod::placeIntern(const osg::Matrix &pose) {
        osg::Matrix p2;
        p2 = pose * osg::Matrix::translate(osg::Vec3(0, 0, conf.diameter / 2));
        create(p2);
    };

    void Sphererobot3MassesMod::sense(GlobalData &globalData) {
    ...
    void Sphererobot3MassesMod::doInternalStuff(GlobalData &global) {
    ...
    int Sphererobot3MassesMod::getMotorNumberIntern() {
        return numberaxis;
    }

    int Sphererobot3MassesMod::getSensorNumberIntern() {
        int s = 0;
    ...
    */
    void Sphererobot3MassesMod::create(const osg::Matrix &pose) {
        if (created) {
            destroy();
        }
     ...
     */
    void Sphererobot3MassesMod::destroy() {
        if (created) {
            for (int i = 0; i < servono; i++) {
    ...
    void Sphererobot3MassesMod::notifyOnChange(const paramkey &key) {
        for (int i = 0; i < servono; i++) {
            if (servo[i])
                servo[i]->setMinMax(-conf.diameter * conf.pendularrange,
                                    conf.diameter * conf.pendularrange);
        }
    }

    std::list<Sensor *> Sphererobot3MassesMod::getSensorList() const {
        return conf.sensors;
    };
